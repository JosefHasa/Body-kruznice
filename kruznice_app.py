...
if radius > 0 and num_points >= 1:
    ...
    c.drawString(2 * cm, height - 4 * cm, f"Stred kružnice: ({x_center}, {y_center}) {unit}")
    c.drawString(2 * cm, height - 5 * cm, f"Polomer: {radius} {unit}")
    c.drawString(2 * cm, height - 6 * cm, f"Pocet bodu: {num_points}")
    c.drawString(2 * cm, height - 7 * cm, f"Barva bodu: {color}")
    c.drawString(2 * cm, height - 8 * cm, f"Spojit body cárou: {'Ano' if show_lines else 'Ne'}")
